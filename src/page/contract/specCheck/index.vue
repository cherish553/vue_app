...
const { postApprovalAdopt, postApprovalBack } = contract
export default {
  components: {
    HistoricalAudit,
    Audit
  ...
  methods: {
    // 型号变更审核通过
    async _postApprovalAdopt () {
      if (await postApprovalAdopt({ id: this.$refs.Audit.records })) return this.$toast('审批通过')
    },
    // 驳回弹框   点击确认
    async _ok () {
      if (!this.reason) return this.$toast('请填写驳回内容')
      if ((await this._postApprovalBack())) this._refresh()
    },
    // 委托批准驳回
    async _postApprovalBack () {
      if (
        await postApprovalBack({
    ...
    async _submit () {
      ...
      if (await this._postApprovalAdopt()) this._refresh()
    },
    // 点击底部   驳回弹窗
    _reject () {
      if (!this.$refs.Audit.records) return this.$toast('至少选择一个')

    ...
    <van-pull-refresh v-model="refresh" @refresh="_onRefresh">
      ...
      >
        <div class="myInfo" v-for="item in recordsList" :key="item.id">
          ...
          <div>审核人: {{item.specAuditor}}</div>
          <div>审核时间: {{item.specAuditorDate|getTime(true)}}</div>
...
const { postExamineHistoryList } = contract
export default {
  data () {
    return {
      pagination: {
  ...
  methods: {
    ...
    async _onLoad () {
      ...
      await this._postExamineHistoryList()
      this.loading = false
      return true
    },
    // 质量历史审核分页列表
    async _postExamineHistoryList () {
      const { records, pages, current } = await postExamineHistoryList(this.pagination)

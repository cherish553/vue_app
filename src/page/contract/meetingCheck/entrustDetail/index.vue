...
export default {
  ...
  methods: {
    ...
    async _submit () {
      ...
      if (await this._postMeetingCheckPass()) this.$back()
    },
    // 点击底部   驳回弹窗
    _reject () {
    ...
    async _ok () {
      ...
      if ((await this._postMeetingCheckBack())) this.$back()
    },
    // 委托会审通过
    async _postMeetingCheckPass () {
      if (await postMeetingCheckPass({ ids: [this.id] })) return this.$toast('会审通过')
    },
    // 委托会审驳回

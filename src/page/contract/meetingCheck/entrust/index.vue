...
const { postMeetingCheckBack, postMeetingCheckPass } = contract
export default {
  data () {
    return {
      Main: 'Main',
  ...
  methods: {
    ...
    async _submit () {
      ...
      if (await this._postMeetingCheckPass()) this._refresh()
    },
    // 点击底部   驳回弹窗
    _reject () {
      if (!this.$refs.Main.records.length) return this.$toast('至少选择一个')
    ...
    async _ok () {
      ...
      if ((await this._postMeetingCheckBack())) this._refresh()
    },
    // 委托会审通过
    async _postMeetingCheckPass () {
      if (await postMeetingCheckPass({ ids: this.$refs.Main.records })) return this.$toast('会审通过')
    },
    // 委托会审驳回
    async _postMeetingCheckBack () {
      if (await postMeetingCheckBack({

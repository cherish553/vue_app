...
const { postQualityMeetingList } = contract
export default {
  name: 'Main',
  data () {
    return {
  ...
  methods: {
    ...
    async _onLoad () {
      ...
      await this._postQualityMeetingList()
      this.loading = false
      return true
    },
    // 全选
    ...
    // 质量委托会审分页列表
    async _postQualityMeetingList () {
      const { records, pages, current } = await postQualityMeetingList(
        this.pagination
      )

...
import Top from './top'
const { postSubpackagePass, postSubpackageBack } = contract
export default {
  components: {
    HistoricalAudit,
    Audit,
  ...
  methods: {
    // 分包申请审批通过
    async _postSubpackagePass () {
      if (await postSubpackagePass({ applyIds: this.$refs.Audit.records })) return this.$toast('审批通过')
    },
    // 驳回弹框   点击确认
    async _ok () {
      if ((await this._postSubpackageBack())) this._refresh()
    },
    // 分包申请审批驳回
    async _postSubpackageBack () {
      if (
        await postSubpackageBack({
          applyIds: this.$refs.Audit.records,
          reason: this.reason
    ...
    async _submit () {
      ...
      if (await this._postSubpackagePass()) this._refresh()
    },
    // 点击底部   驳回弹窗
    _reject () {
      if (!this.$refs.Audit.records.length) return this.$toast('至少选择一个')

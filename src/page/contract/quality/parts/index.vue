...
export default {
  ...
  methods: {
    ...
    async _postQualityPass () {
      if (await postQualityPass({ ids: this.$refs.Main.records, serveGrade: this.$refs.modal1.value })) return this.$toast('审核通过')
    },
    // 获取服务器等级
    async _postDictList () {
    ...
    async _oks () {
      ...
      if (!await this._postQualityPass()) return
    ...
    async _ok () {
      ...
      if (await this._postQualityback()) this._refresh()
    },
    // 质量部审核驳回

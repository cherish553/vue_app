    ...
    >
      <el-checkbox-group v-model="records">
        <div class="myInfo" v-for="item in recordsList" :key="item.id">
          ...
          <div
            class="myInfo-inner"
            @click="$jump('quality_department_detail',{id:item.id})"
          >
...
const { postQualityList } = contract
export default {
  name: 'Main',
  data () {
    return {
  ...
  methods: {
    ...
    // 质量部审核分页
    async _postQualityList () {
      const { records, pages, current } = await postQualityList(
        this.pagination
      )

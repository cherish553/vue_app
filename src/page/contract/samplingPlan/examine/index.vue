...
import Top from './top'
const { postSpotPass, postSpotBack } = contract
export default {
  components: {
    Main,
    Top
  ...
  methods: {
    //  抽检 根据 ids 提交
    async _postSpotPass () {
      if (await postSpotPass({ ids: this.$refs.Main.records })) return this.$toast('审批通过')
    },
    // 驳回弹框   点击确认
    async _ok () {
    ...
    async _submit () {
      ...
      if (await this._postSpotPass()) this._refresh()
    },
    // 点击底部   驳回弹窗
    _reject () {
      if (!this.$refs.Main.records.length) return this.$toast('至少选择一个')

    ...
    >
      <el-checkbox-group v-model="records">
        <div class="myInfo" v-for="item in recordsList" :key="item.id">
          ...
          <div
            class="myInfo-inner"
            @click="$jump('contract_approve_detail',{id:item.id})"
          >
            ...
            <div>会审通过日期: {{item.meetCheckDate|getTime(true)}}</div>
          </div>
        </div>
      </el-checkbox-group>
    </van-list>
...
const { postQualityApproveList } = contract
export default {
  name: 'Main',
  data () {
    return {
  ...
  methods: {
    ...
    async _onLoad () {
      ...
      await this._postQualityApproveList()
      this.loading = false
      return true
    },
    // 全选
    ...
    // 质量委托批准分页列表
    async _postQualityApproveList () {
      const { records, pages, current } = await postQualityApproveList(
        this.pagination
      )

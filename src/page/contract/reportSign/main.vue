    ...
    >
      <el-checkbox-group v-model="records">
        <div class="myInfo" v-for="item in recordsList" :key="item.id">
          ...
          <div
          @click="$jump('report_sign_detail',{id:item.id})"
            class="myInfo-inner"
          >
...
const { postReportMeetingList } = contract
export default {
  name: 'Main',
  data () {
    return {
  ...
  methods: {
    ...
    async _onLoad () {
      ...
      await this._postReportMeetingList()
      this.loading = false
      return true
    },
    // 全选
    ...
    // 报告会签分页列表
    async _postReportMeetingList () {
      const { records, pages, current } = await postReportMeetingList(
        this.pagination
      )

...
import Top from './top'
const { postReportMeetingPass, postSpotBack } = contract
export default {
  components: {
    Main,
    Top
  ...
  data () {
    return {
      ...
      option: [
        ...
        {
          name: '申诉', color: '#ffa721', methods: '_reject'
        }
      ]
    }
  },
  methods: {
    ...
    async _postReportMeetingPass () {
      if (await postReportMeetingPass({ ids: this.$refs.Main.records })) return this.$toast('审批通过')
    },
    // 驳回弹框   点击确认
    async _ok () {
      if (!this.reason) return this.$toast('请填写驳回内容')
      if ((await this._postSpotBack())) this._refresh()
    },
    //  抽检 根据 ids 驳回
    async _postSpotBack () {
      if (
    ...
    _reject () {
      if (!this.$refs.Main.records.length) return this.$toast('至少选择一个')
      this.$jump('report_sign_appeal', { id: this.$refs.Main.records.join(',') })
    },
    // 刷新列表并且关闭弹框
    _refresh () {
      this.show = false
